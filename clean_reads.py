...
def sample_info(args):
	...
	row = d.ix[d['SAMPLE_ID'] == args.sample, ].to_dict('list')
...
def adaptor_file(args, info, dir):
	...
	seqfile = os.path.join(args.dir, 'metadata', 'oligo_sequences.fasta')
	adfile = os.path.join(args.dir, 'metadata', 'inline_barcodes.csv')
	b = pd.read_csv(adfile)

	ad = get_seq(seqfile)

	o = open(a_file, 'w')
	# want to write (unconditional) ILLpcr1 and ad2
	o.write('>%s\n%s\n' % ('ILLpcr1', ad['ILLpcr1']))
	o.write('>%s_rc\n%s\n' % ('ILLpcr1', rc(ad['ILLpcr1'])))
	o.write('>%s\n%s\n' % ('ad2', ad['ad2']))
        o.write('>%s_rc\n%s\n' % ('ad2', rc(ad['ad2'])))
	
	# want to write conditional illpcr2 and ad1
	pcr2 = info['PCR_INDEX']
	if pd.isnull(pcr2):
		pcr2 = 'pcr2'
	o.write('>%s\n%s\n' % (pcr2, ad[pcr2]))
	...
	barcode = b.ix[b.inline_barcode_ID == info['INLINE_BARCODE'], 'index'].tolist()
	if len(barcode) > 0:
		barcode = re.sub('\*', barcode[0], ad['ad1'])
	else:
		barcode = re.sub('\*', '', ad['ad1'])
	o.write('>ad\n%s\n' % (barcode))
	o.write('>ad_rc\n%s\n' % (rc(barcode)))
	o.close()
...
def run_trimmomatic(args, info, a_file, dir):
	...
	read1 = os.path.join(args.dir, 'ind_reads', '%s.1.noRE.fq.gz' % args.sample)
	read2 = os.path.join(args.dir, 'ind_reads', '%s.2.noRE.fq.gz' % args.sample)
	subprocess.call("java -jar %s PE -threads %s -phred33 %s %s %s ILLUMINACLIP:%s:2:30:10"  %
                        (args.trimjar, args.CPU, read1, read2, ' '.join(outfiles), 
                        a_file), shell=True)
